<template>
  <div class="container" id="change-password" @mousedown="getElemetnId" @keyup.tab="getElemetnId">
    <div class="card shadow mt-5">
      ...
      <p class="mr-3 mt-2">تغییر رمز عبور</p>
      <hr style="width: 100%; margin-top: -0.5em" />
      <div id="input-group" @click="code = 2" @keyup.tab="code = 2">
        <!-- <div class="d-flex flex-column" v-if="errors.length != 0">
          <transition name="fade">
            <span class="text-danger" v-if="errors[1] == 0">- زمز عبور خود را وارد نمایید</span>
            <span class="text-danger" v-if="errors[1] == 1">- رمز عبور اشتباه است</span>
          </transition>
        </div>-->
        <div class="d-flex flex-column">
          ...
          >
            <input
              ...
              ref="oldPassword"
            />
            <span :class="{ 'place-holder-transition': activePlaceHolder[0] }">رمز قبلی</span>
          </div>
          <div
            class="input_place-holder row mt-2"
      ...
      <router-link to="/profile/personal_info" class="ml-1 mb-2 pr-3 pl-3">
        <button type="button" class="btn mt-4" id="btn-enter">تغییر رمز ورود</button>
      </router-link>
    </div>
  </div>
</template>
